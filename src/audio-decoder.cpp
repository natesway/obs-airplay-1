...
auto AudioDecoder::decode(std::span<const uint8_t> data) -> const AFrame *
{
  auto c = [=]() {
    ...
    blog(LOG_INFO,"Unknown audio codec: %i", data[0]);
    return AudioCodec::unsupported;
  }();

  if (c == AudioCodec::alac || c == AudioCodec::unsupported)
  {
    blog(LOG_INFO,"audio-format is not supported");
    return nullptr;
  }
  ...
  UINT bytesValid = data.size();
  {
    ...
    if (err != AAC_DEC_OK)
    {
      blog(LOG_INFO,"aacDecoder_Fill failed: %c", err);
      return nullptr;
    }
  }
  {
    ...
    if (err != AAC_DEC_OK)
    {
      blog(LOG_INFO,"aacDecoder_DecodeFrame failed: %c", err);
      return nullptr;
    }
  }
  {
    ...
    if (info == nullptr)
    {
      blog(LOG_INFO,"aacDecoder_GetStreamInfo failed");
      return nullptr;
    }
    ...
    case 2: // stereo
      ...
      break;
    default: blog(LOG_INFO,"Unknown channel config: %d", info->channelConfig); return nullptr;
    }
...
{
  {
    ...
    if (err != AAC_DEC_OK)
    {
      blog(LOG_INFO,"aacDecoder_ConfigRaw failed: %d", err);
      return;
    }
  }
}
