                                ...
                                bool debug_log) -> int
{
  ...
  if (raop == NULL)
  {
    blog(LOG_INFO,"Error initializing raop!");
    return -1;
  }
  ...
  if (error)
  {
    blog(LOG_INFO,"Could not initialize dnssd library!");
    stop_raop_server();
    return -2;
  }
...
auto AirPlay::conn_init(void *cls) -> void
{
  ...
  blog(LOG_INFO, "Open connections: %d", self->open_connections);
}
...
{
  auto self = static_cast<AirPlay *>(cls);
  blog(LOG_INFO,"***ERROR lost connection with client (network problem?)");
  if (timeouts)
  {
    blog(LOG_INFO,"   Client no-response limit of %d timeouts (%d seconds) reached:", timeouts, 3 * timeouts);
    blog(LOG_INFO,"   Sometimes the network connection may recover after a longer delay:\n"
        "   the default timeout limit n = %d can be changed with the \"-reset n\" option",
        NTP_TIMEOUT_LIMIT);
  }
  blog(LOG_INFO,"reset_video %d", reset_video);
  raop_stop(self->raop);
}
  ...
  if (buflen > 4)
  {
    ...
    for (int i = 4; i < buflen; i++)
    {
      if (memcmp(metadata, mark, 3) == 0 && (len = (int)*(metadata + 3)))
      {
        ...
        {
          blog(LOG_INFO,"Artist: ");
        }
        else if (strcmp(tag, "ascp") == 0)
        {
          blog(LOG_INFO,"Composer: ");
        }
    ...
    obsAFrame(std::make_unique<obs_source_audio>())
{
  ...
  if (mac_address.empty())
  {
    ...
    blog(LOG_INFO,"using randomly-generated MAC address %s", mac_address);
  }
  else
  {
    blog(LOG_INFO,"using system MAC address %s", mac_address);
  }
  ...
  {
    blog(LOG_INFO,"start_raop_server failed");
    return;
  }
  counter = 0;
  compression_type = 0;
